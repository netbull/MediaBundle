...
class FormatThumbnail implements ThumbnailInterface
{
    ...
    /**
     * @param bool $fork when true (default), each format is generated by forking a
     *     `netbull:media:create-thumbnail` subprocess — isolating memory, which matters for
     *     long-running web requests. When false the resize runs in-process: far faster for bulk
     *     CLI work that already isolates memory per item. Toggle via `netbull_media.thumbnail.fork`.
     */
    public function __construct(ParameterBagInterface $parameterBag, LoggerInterface $logger, bool $fork = true)
    {
    ...
    public function generate(MediaProviderInterface $provider, MediaInterface $media): void
    {
        ...
        foreach ($provider->getFormats() as $format => $settings) {
            ...
            if (!$this->fork) {
                // In-process resize — no subprocess kernel boot. Identical output to the forked
                // path; enabled via `netbull_media.thumbnail.fork: false`.
